...
from app.v9_lab import build_v9_supreme, V9_NAME
from contextlib import asynccontextmanager
from apscheduler.schedulers.background import BackgroundScheduler
from bs4 import BeautifulSoup
import httpx, json, math, os, re, hashlib, threading, time, csv, io, hmac

APP_NAME = "OJO-99 V9 Supreme Lab Safe"
...
HISTORY_TURBO_ENABLED = os.getenv("HISTORY_TURBO_ENABLED", "false").strip().lower() == "true"
HISTORY_TURBO_MINUTES = max(10, int(os.getenv("HISTORY_TURBO_MINUTES", "15")))
...
ADMIN_RATE_LIMIT_PER_MINUTE = max(1, int(os.getenv("ADMIN_RATE_LIMIT_PER_MINUTE", "20")))
_admin_rate_lock = threading.Lock()
_admin_rate = defaultdict(list)
...
def _client_ip(request):
    ...
    return xf.split(",")[0].strip() if xf else (request.client.host if request.client else "unknown")

def _admin_rate_ok(ip):
    now = time.time()
    with _admin_rate_lock:
        arr = [x for x in _admin_rate[ip] if now-x < 60]
        if len(arr) >= ADMIN_RATE_LIMIT_PER_MINUTE:
            _admin_rate[ip] = arr
            return False
        arr.append(now); _admin_rate[ip] = arr
        return True

def require_admin(request):
    # Solo protege escrituras manuales. Los procesos automáticos internos quedan intactos.
    if not ADMIN_API_KEY:
        raise HTTPException(503, "Administración protegida: falta configurar ADMIN_API_KEY.")
    if not _admin_rate_ok(_client_ip(request)):
        raise HTTPException(429, "Demasiadas operaciones administrativas.")
    supplied = request.headers.get("x-ojo99-admin-key", "")
...
def parse_spanish_long_date(s):
    ...
    m = re.search(r"(\d{1,2})\s+de\s+([a-záéíóúñ]+)\s+de\s+(\d{4})", x)
    if not m:
        return None
    dd = int(m.group(1))
    month_name = m.group(2)
    mm = SPANISH_MONTHS.get(month_name)
    yyyy = int(m.group(3))
    if not mm:
        return None
    try:
...
def extract_anguilla_manana_history(html):
    ...
    La página histórica usa fechas largas en español.
    """
    ...
    for i, line in enumerate(lines):
        ...
        # Buscar exactamente 3 números después de la hora
        start_idx = window.index(time_s) + 1
        nums = []
        ...
        t = datetime.strptime("10:00 AM", "%I:%M %p")
        dt = datetime(
                ...
                try:
                    ...
                    with SessionLocal() as db:
                        ...
                        set_cache(
                            ...
                            len(rows),
                            (
                                f"game={game}; date={label}; rows={len(rows)}; "
                                f"new={new}; page={page_used}"
                            )
                        )
                    ...
                    with SessionLocal() as db:
                        fresh=db.scalar(
                            select(SourceRegistry).where(SourceRegistry.key==src.key)
                        )
...
def lottery_performance_ranking():
    """
    Ranking por resultados REALES de predicciones congeladas antes del sorteo.
    """
    rows = []

    for cfg in LOTTERY_SCHEDULES:
        lottery = cfg["name"]
        p = performance(lottery)
        evaluated = int(p.get("evaluated") or 0)
        top5 = p.get("top5_numbers")
        two3 = p.get("two_of_three")
        pale = p.get("top5_pairs")
        trip = p.get("top5_triples")

        sample_factor = min(1.0, evaluated / 100.0)
        raw_score = (
            (float(top5) if top5 is not None else 0.0) * 0.35 +
            (float(two3) if two3 is not None else 0.0) * 0.30 +
            (float(pale) if pale is not None else 0.0) * 0.25 +
            (float(trip) if trip is not None else 0.0) * 0.10
        )
        ranking_score = round(raw_score * sample_factor, 2)
        ...
        rows.append({
            ...
            "top5_numbers_pct": top5,
            "two_of_three_pct": two3,
            "pale_pct": pale,
            "tripleta_pct": trip,
            "ranking_score": ranking_score
        })

    rows.sort(key=lambda x: (x["ranking_score"], x["evaluated"]), reverse=True)

    for i,row in enumerate(rows, start=1):
        row["rank"] = i
        if row["evaluated"] < 30:
            row["status"] = "EN ESTUDIO"
        elif row["ranking_score"] >= 35:
            row["status"] = "FUERTE"
        elif row["ranking_score"] >= 20:
            row["status"] = "UTILIZABLE"
        else:
            row["status"] = "DEBIL"
...
def data_freshness_status():
    """
    Vigila la última fecha disponible por lotería.
    """
    ...
    with SessionLocal() as db:
        for cfg in LOTTERY_SCHEDULES:
            last = db.scalar(
                select(CanonicalDraw.draw_time)
                .where(
                    CanonicalDraw.lottery == cfg["name"],
                    CanonicalDraw.verification_state != "CONFLICT"
                )
                .order_by(desc(CanonicalDraw.draw_time))
                .limit(1)
            )
...
def auto_freeze_due_predictions():
    """
    Congela automáticamente SOLO entre 5 y 25 minutos antes del sorteo.
    Usa freeze_prediction(), por lo que no genera números al azar.
    """
    now = datetime.now(DR_TZ)
    results = []
    ...
    for cfg in LOTTERY_SCHEDULES:
        ...
        if 5 <= minutes_to <= 25:
            try:
                result = freeze_prediction(cfg["name"])
                results.append({
                    "lottery": cfg["name"],
                    "minutes_to_draw": round(minutes_to, 1),
                    "result": result
                })
            except Exception as e:
                results.append({
    ...
    return {"at": now.isoformat(), "processed": results}
...
async def lifespan(app):
    ...
    if COLLECTOR_ENABLED and not scheduler.running:
        ...
        scheduler.add_job(
            ...
            "interval",
            minutes=(HISTORY_TURBO_MINUTES if HISTORY_TURBO_ENABLED else HISTORY_SYNC_MINUTES),
            id="history",
            replace_existing=True,
            max_instances=1,
            coalesce=True
...
@app.middleware("http")
async def escudo_v1_headers(request: Request, call_next):
...
def security_status():
    return {
        ...
        "admin_rate_limit_per_minute":ADMIN_RATE_LIMIT_PER_MINUTE,
        "max_csv_bytes":MAX_CSV_BYTES,
        "historical_collector_untouched":True,
        "database_schema_untouched":True,
        "history_turbo_enabled":HISTORY_TURBO_ENABLED,
        "history_interval_minutes":(
            HISTORY_TURBO_MINUTES if HISTORY_TURBO_ENABLED
            else HISTORY_SYNC_MINUTES
        )
    }


@app.get("/api/network/turbo-status")
def turbo_status():
    return {
        "turbo_enabled": HISTORY_TURBO_ENABLED,
        "mode": "TURBO_SEGURO" if HISTORY_TURBO_ENABLED else "NORMAL",
        "history_interval_minutes": (
            HISTORY_TURBO_MINUTES if HISTORY_TURBO_ENABLED
            else HISTORY_SYNC_MINUTES
        ),
        "safe_min_seconds_between_requests": SAFE_MIN_SECONDS_BETWEEN_REQUESTS,
        "backoff_base_minutes": SAFE_BACKOFF_BASE_MINUTES,
        "backoff_max_minutes": SAFE_BACKOFF_MAX_MINUTES,
        "cache_enabled": True,
        "circuit_breaker_enabled": True,
        "note": "Turbo acelera la frecuencia de rondas; respeta cache, pausas y backoff ante 403/429."
    }




@app.get("/api/radar-top20")
def api_radar_top20(lottery:str):
    if lottery not in SCHEDULE_MAP:
        raise HTTPException(404, "Lotería no registrada.")
    with SessionLocal() as db:
        draws = canonical_draws(db, lottery)
    return build_top20_radar(lottery, draws)


@app.get("/api/v9/status")
def v9_status():
    return {
        "version":V9_NAME,
        "mode":"READ_ONLY_LAB",
        "database_schema_changed":False,
        "historical_collector_changed":False,
        "turbo_changed":False,
        "escudo_changed":False,
        "background_lab_jobs":False,
...
def build_top20_radar(lottery, draws):
    ...
    Usa el MISMO ranking calibrado de números que V9/Top 5.
    No escribe en DB, no modifica histórico, no crea números aleatorios.
    """
    if len(draws) < 30:
        return {
...
def analyze(lottery:str):
    ...
    with SessionLocal() as db:
        draws=canonical_draws(db,lottery)
    out=robust_build_analysis(lottery,draws)
    out["lottery"]=lottery
    out["schedule"]=SCHEDULE_MAP[lottery]
    out["v9"]=build_v9_supreme(lottery,SCHEDULE_MAP[lottery]["numbers"],out)
    out["radar_top20"]=build_top20_radar(lottery, draws)
    return out
...
@app.get("/api/ranking")
def api_ranking():
    return {"ranking": lottery_performance_ranking()}


@app.get("/api/freshness")
def api_freshness():
    return {"lotteries": data_freshness_status()}


@app.post("/api/auto-freeze-now")
def api_auto_freeze_now(request: Request):
    require_admin(request)
    return auto_freeze_due_predictions()
...
def api_freeze(request: Request, lottery:str=Form(...)):
    require_admin(request)
...
def history_step(request: Request):
    require_admin(request)
...
async def import_csv(request: Request, file:UploadFile=File(...)):
    ...
    if len(data) > MAX_CSV_BYTES:
        raise HTTPException(413, f"CSV demasiado grande. Máximo {MAX_CSV_BYTES} bytes.")
...
def manual_draw(request: Request, lottery:str=Form(...),draw_time:str=Form(...),numbers:str=Form(...)):
    require_admin(request)
