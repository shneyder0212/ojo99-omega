...
def extract_public_results(html):
    ...
    for i,line in enumerate(lines):
        ...
        try: dt=dr_local_to_utc(date_s,time_s)
        except: continue
...
def collect_history_step():
    ...
    with SessionLocal() as db:
        ...
        if cur.finished: return {"status":"FINISHED"}
        day=date.fromisoformat(cur.cursor_date); target=date.fromisoformat(cur.target_date)
        sources=db.scalars(select(SourceRegistry).where(SourceRegistry.enabled==True)).all()
    if day<target:
        with SessionLocal() as db:
            cur=ensure_cursor(db); cur.finished=True; db.commit()
        return {"status":"FINISHED"}
    out={"date":day.isoformat(),"sources":[]}
    with collector_lock:
        for src in sources:
            with SessionLocal() as db: c=get_cache(db,src.key,day)
            if c and c.status in ("COMPLETE","EMPTY"):
                out["sources"].append({"source":src.key,"status":"CACHED"}); continue
            if source_paused(src):
                out["sources"].append({"source":src.key,"status":"PAUSED"}); continue
            label=day.strftime("%d-%m-%Y"); sep="&" if "?" in src.url else "?"
            url=f"{src.url}{sep}date={label}"
            try:
                rows=fetch_source(src,url); touched=set(); new=0
                with SessionLocal() as db:
                    for game,dt,nums in rows:
                        if ingest_observation(db,src.key,game,dt,nums,url): new+=1
                        touched.add((game,dt))
                    for game,dt in touched: rebuild_canonical(db,game,dt)
                    db.commit(); set_cache(db,src.key,day,"COMPLETE" if rows else "EMPTY",len(rows),f"new={new}")
                out["sources"].append({"source":src.key,"status":"OK","rows":len(rows)})
            except Exception as e:
                with SessionLocal() as db: set_cache(db,src.key,day,"ERROR",0,str(e))
                out["sources"].append({"source":src.key,"status":"ERROR","message":str(e)[:160]})
    with SessionLocal() as db:
        cur=ensure_cursor(db); nxt=day-timedelta(days=1)
        cur.cursor_date=nxt.isoformat(); cur.finished=nxt<date.fromisoformat(cur.target_date); db.commit()
    return out
...
async def lifespan(app):
    ...
    if COLLECTOR_ENABLED and not scheduler.running:
        scheduler.add_job(collect_current,"interval",minutes=CURRENT_SYNC_MINUTES,id="current",replace_existing=True,max_instances=1,coalesce=True)
        scheduler.add_job(collect_history_step,"interval",minutes=HISTORY_SYNC_MINUTES,id="history",replace_existing=True,max_instances=1,coalesce=True)
        scheduler.start()
    yield
