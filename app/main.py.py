...
def collect_history_step():
    """
    HISTORICAL FIX V8.1
    - procesa hasta 3 días por ejecución;
    - avanza el cursor solo tras procesar cada fecha;
    - respeta caché COMPLETE/EMPTY;
    - filtra estrictamente por la fecha solicitada;
    - mantiene el recolector seguro.
    """
    seed_sources()
    ...
    for _ in range(3):
        ...
        day_result = {"date": day.isoformat(), "sources": []}

        with collector_lock:
            for src in sources:
                if source_paused(src):
                    day_result["sources"].append({
                        "source": src.key,
                        "status": "PAUSED"
                    })
                    continue
                ...
                try:
                    ...
                    # Aceptar solo sorteos cuya fecha realmente coincide
                    # con el día histórico solicitado.
                    filtered_rows = []
                    for game, dt, nums in rows:
                        if dt.astimezone(DR_TZ).date() == day:
                            filtered_rows.append((game, dt, nums))

                    touched = set()
                    new_obs = 0

                    with SessionLocal() as db:
                        for game, dt, nums in filtered_rows:
                            if ingest_observation(
                                db,
                                src.key,
                                game,
                                dt,
                        ...
                        set_cache(
                            ...
                            src.key,
                            day,
                            "COMPLETE" if filtered_rows else "EMPTY",
                            len(filtered_rows),
                            (
                                f"requested={label}; "
                                f"parsed={len(rows)}; "
                                f"matched_date={len(filtered_rows)}; "
                                f"new={new_obs}"
                            )
                        )
                    ...
                    day_result["sources"].append({
                        ...
                        "status": "OK",
                        "parsed": len(rows),
                        "matched_date": len(filtered_rows),
                        "new": new_obs
                    })
        ...
        processed_days.append(day_result)

        # Avanzar un día después de haber procesado esa fecha.
        with SessionLocal() as db:
            cur = ensure_cursor(db)
            current_cursor = date.fromisoformat(cur.cursor_date)

            if current_cursor == day:
                next_day = day - timedelta(days=1)
                cur.cursor_date = next_day.isoformat()
                cur.finished = next_day < date.fromisoformat(cur.target_date)
                db.commit()

        statuses = [x.get("status") for x in day_result["sources"]]
        if statuses and all(s in ("PAUSED", "ERROR") for s in statuses):
            break
...
async def lifespan(app):
    ...
    if COLLECTOR_ENABLED and not scheduler.running:
        ...
        # Ejecutar una sincronización actual y una histórica
